...
class nnUNetTrainerV2_DA5(nnUNetTrainerV2):
    ...
    def setup_DA_params(self):
        ...
        if self.threeD:
            ...
            if self.do_dummy_2D_aug:
                self.print_to_log_file("Using dummy2d dataloader augmentation")
    ...
    def get_train_transforms(self) -> List[AbstractTransform]:
        ...
        if np.any(matching_axes > 1):
            tr_transforms.append(
                Rot90Transform(
                    (0, 1, 2, 3), axes=valid_axes, data_key='dataloader', label_key='seg', p_per_sample=0.5
                ),
            )

        if np.any(matching_axes > 1):
            tr_transforms.append(
                TransposeAxesTransform(valid_axes, data_key='dataloader', label_key='seg', p_per_sample=0.5)
            )
        ...
        if self.data_aug_params["move_last_seg_chanel_to_data"]:
            ...
            tr_transforms.append(MoveSegAsOneHotToData(1, all_class_labels, 'seg', 'dataloader'))
    ...
    def get_val_transforms(self) -> List[AbstractTransform]:
        ...
        if self.data_aug_params["move_last_seg_chanel_to_data"]:
            all_class_labels = np.arange(1, self.num_classes)
            val_transforms.append(MoveSegAsOneHotToData(1, all_class_labels, 'seg', 'dataloader'))
        ...
        if not self.was_initialized:
            ...
            if training:
                ...
                if self.unpack_data:
                    ...
                    print(
                        "INFO: Not unpacking dataloader! Training may be slow due to that. Pray you are not using 2d or you "
