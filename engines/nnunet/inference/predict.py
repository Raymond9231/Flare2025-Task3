...
def preprocess_save_to_queue(preprocess_fn, q, list_of_lists, output_files, segs_from_prev_stage, classes,
    ...
    for i, l in enumerate(list_of_lists):
        try:
            ...
            """There is a problem with python preprocess communication that prevents us from communicating objects 
            ...
            if np.prod(d.shape) > (2e9 / 4 * 0.85):  # *0.85 just to be save, 4 because float32 is 4 bytes
                print(
                    "This output is too large for python preprocess-preprocess communication. "
...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
    ...
    for preprocessed in preprocessing:
        ...
        if np.prod(softmax.shape) > (2e9 / bytes_per_voxel * 0.85):  # * 0.85 just to be save
            print(
                "This output is too large for python preprocess-preprocess communication. Saving output temporarily to disk")
...
def predict_cases_fast(model, list_of_lists, output_filenames, folds, num_threads_preprocessing,
    ...
    print("starting prediction...")
    for preprocessed in preprocessing:
        print("getting dataloader from preprocessor")
        output_filename, (d, dct) = preprocessed
        print("got something")
        if isinstance(d, str):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--part_id", type=int, required=False, default=0, help="Used to parallelize the prediction of "
                                                                               ...
                                                                               "want to use n GPUs to inference this "
    ...
    "Determines many background processes will be used for dataloader preprocessing. Reduce this if you "
    "run into out of memory (RAM) problems. Default: 6")
    parser.add_argument("--num_threads_nifti_save", required=False, default=2, type=int, help=
    "Determines many background processes will be used for segmentation export. Reduce this if you "
    "run into out of memory (RAM) problems. Default: 2")
    parser.add_argument("--tta", required=False, type=int, default=1, help="Set to 0 to disable test time dataloader "
