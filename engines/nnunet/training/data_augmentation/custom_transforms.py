...
class MaskTransform(AbstractTransform):
    def __init__(self, dct_for_where_it_was_used, mask_idx_in_seg=1, set_outside_to=0, data_key="dataloader", seg_key="seg"):
        """
        dataloader[mask < 0] = 0
...
def convert_3d_to_2d_generator(data_dict):
    shp = data_dict['dataloader'].shape
    data_dict['dataloader'] = data_dict['dataloader'].reshape((shp[0], shp[1] * shp[2], shp[3], shp[4]))
...
def convert_2d_to_3d_generator(data_dict):
    ...
    current_shape = data_dict['dataloader'].shape
    data_dict['dataloader'] = data_dict['dataloader'].reshape((shp[0], shp[1], shp[2], current_shape[-2], current_shape[-1]))
