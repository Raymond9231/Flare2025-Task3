...
# sometimes you have 3d dataloader and a 3d net but cannot augment them properly in 3d due to anisotropy (which is currently
# not supported in batchgenerators). In that case you can 'cheat' and transfer your 3d dataloader into 2d dataloader and
...
def get_default_augmentation(dataloader_train, dataloader_val, patch_size, params=default_3D_augmentation_params,
    ...
    # don't do color augmentations while in 2d mode with 3d dataloader because the color channel is overloaded!!
    ...
    if params.get("move_last_seg_chanel_to_data") is not None and params.get("move_last_seg_chanel_to_data"):
        tr_transforms.append(MoveSegAsOneHotToData(1, params.get("all_segmentation_labels"), 'seg', 'dataloader'))
    ...
    tr_transforms.append(NumpyToTensor(['dataloader', 'target'], 'float'))
    ...
    if params.get("move_last_seg_chanel_to_data") is not None and params.get("move_last_seg_chanel_to_data"):
        val_transforms.append(MoveSegAsOneHotToData(1, params.get("all_segmentation_labels"), 'seg', 'dataloader'))
    ...
    val_transforms.append(NumpyToTensor(['dataloader', 'target'], 'float'))
