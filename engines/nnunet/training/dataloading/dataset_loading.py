...
def unpack_dataset(folder, threads=default_num_threads, key="dataloader"):
    """
...
def pack_dataset(folder, threads=default_num_threads, key="dataloader"):
    p = Pool(threads)
...
def load_dataset(folder, num_cases_properties_loading_threshold=1000):
    # we don't load the actual dataloader but instead return the filename to the np file.
...
def crop_2D_image_force_fg(img, crop_size, valid_voxels):
    ...
    if type(crop_size) not in (tuple, list):
        ...
        assert len(crop_size) == (len(
            img.shape) - 1), "If you provide a list/tuple as center crop make sure it has the same len as your dataloader has dims (3d)"
...
class DataLoader3D(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
                 ...
                 pad_sides=None):
        """
        This is the basic dataloader loader for 3D networks. It uses preprocessed dataloader as produced by my (Fabian) preprocessing.
        You can load the dataloader with load_dataset(folder) where folder is the folder where the npz files are located. If there
        are only npz files present in that folder, the dataloader loader will unpack them on the fly. This may take a while
        ...
        dataloader is stored in a compressed format (factor 10 smaller) and only unpacked when needed.
        :param data: get this with load_dataset(folder, stage=0). Plug the return value in here and you are g2g (good to go)
        :param patch_size: what patch size will this dataloader loader return? it is common practice to first load larger
        patches so that a central crop after dataloader augmentation can be done to reduce border artifacts. If unsure, use
        get_patch_size() from data_augmentation.default_data_augmentation
        :param final_patch_size: what will the patch finally be cropped to (after dataloader augmentation)? this is the patch
        ...
        :param batch_size:
        :param num_batches: how many batches will the dataloader loader produce before stopping? None=endless
        :param seed:
        ...
        # need_to_pad denotes by how much we need to pad the dataloader so that if we sample a patch of size final_patch_size
    ...
    def determine_shapes(self):
        ...
        if isfile(self._data[k]['data_file'][:-4] + ".npy"):
            ...
            case_all_data = np.load(self._data[k]['data_file'])['dataloader']
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            if isfile(self._data[i]['data_file'][:-4] + ".npy"):
                ...
                case_all_data = np.load(self._data[i]['data_file'])['dataloader']
            ...
            # applied to it in segmentation mode. Later in the dataloader augmentation we move it from the segmentations to
            # the last channel of the dataloader
            if self.has_prev_stage:
                if isfile(self._data[i]['seg_from_prev_stage_file'][:-4] + ".npy"):
                    ...
                    segs_from_previous_stage = np.load(self._data[i]['seg_from_prev_stage_file'])['dataloader'][None]
            ...
            # whoever wrote this knew what he was doing (hint: it was me). We first crop the dataloader to the region of the
            # bbox that actually lies within the dataloader. This will result in a smaller array which is then faster to pad.
            # valid_bbox is just the coord that lied within the dataloader cube. It will be padded to match the patch size
            # later
            ...
            # remove label -1 in the dataloader augmentation but this way it is less error prone)
        ...
        return {'dataloader': data, 'seg': seg, 'properties': case_properties, 'keys': selected_keys}
...
class DataLoader2D(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, oversample_foreground_percent=0.0,
        ...
        """
        This is the basic dataloader loader for 2D networks. It uses preprocessed dataloader as produced by my (Fabian) preprocessing.
        You can load the dataloader with load_dataset(folder) where folder is the folder where the npz files are located. If there
        are only npz files present in that folder, the dataloader loader will unpack them on the fly. This may take a while
        and increase CPU usage. Therefore, I advise you to call unpack_dataset(folder) first, which will unpack all npz
        to npy. Don't forget to call delete_npy(folder) after you are done with training?
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            if not isfile(self._data[i]['data_file'][:-4] + ".npy"):
                ...
                case_all_data = np.load(self._data[i]['data_file'][:-4] + ".npz")['dataloader']
            else:
                case_all_data = np.load(self._data[i]['data_file'][:-4] + ".npy", self.memmap_mode)
            ...
            # case all dataloader should now be (c, x, y)
        ...
        keys = selected_keys
        return {'dataloader': data, 'seg': seg, 'properties': case_properties, "keys": keys}
