...
class NeuralNetwork(nn.Module):
    ...
    def get_device(self):
        if next(self.parameters()).device.train_type == "cpu":
            return "cpu"
        else:
...
class SegmentationNetwork(NeuralNetwork):
    ...
    def predict_3D(self, x: np.ndarray, do_mirroring: bool, mirror_axes: Tuple[int, ...] = (0, 1, 2),
        ...
        """
        Use this function to inference a 3D image. It does not matter whether the network is a 2D or 3D U-Net, it will
        detect that automatically and run the appropriate code.

        When running predictions, you need to specify whether you want to run fully convolutional of sliding window
        based inference. We very strongly recommend you use sliding window with the default settings.

        It is the responsibility of the user to make sure the network is in the proper mode (val for inference!). If
        the network is not in val mode it will print a warning.

        :param x: Your input dataloader. Must be a nd.ndarray of shape (c, x, y, z).
        :param do_mirroring: If True, use test time dataloader augmentation in the form of mirroring
        ...
        assert len(x.shape) == 4, "dataloader must have shape (c,x,y,z)"
    ...
    def predict_2D(self, x, do_mirroring: bool, mirror_axes: tuple = (0, 1, 2), use_sliding_window: bool = False,
                   ...
                   verbose: bool = True, mixed_precision: bool = True) -> Tuple[np.ndarray, np.ndarray]:
        """
        Use this function to inference a 2D image. If this is a 3D U-Net it will crash because you cannot inference a 2D
        ...
        :param x: Your input dataloader. Must be a nd.ndarray of shape (c, x, y).
        :param do_mirroring: If True, use test time dataloader augmentation in the form of mirroring
        :param mirror_axes: Determines which axes to use for mirroing. Per default, mirroring is done along all three
        axes
        :param use_sliding_window: if True, run sliding window prediction. Heavily recommended! This is also the default
        ...
        if self.conv_op == nn.Conv3d:
            raise RuntimeError("Cannot inference 2d if the network is 3d. Dummy.")
        ...
        assert len(x.shape) == 3, "dataloader must have shape (c,x,y)"
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        if verbose:
            print("dataloader shape:", data_shape)
        ...
        if use_gaussian and num_tiles > 1:
            ...
            #inference on cpu if cuda not available
        ...
        if all_in_gpu:
            ...
            if verbose: print("moving dataloader to GPU")
    ...
    def _internal_predict_3D_2Dconv(self, x: np.ndarray, min_size: Tuple[int, int], do_mirroring: bool,
        ...
        if all_in_gpu:
            raise NotImplementedError
        assert len(x.shape) == 4, "dataloader must be c, x, y, z"
