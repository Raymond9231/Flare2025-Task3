...
def save_segmentation_nifti_from_softmax(segmentation_softmax: Union[str, np.ndarray], out_fname: str,
    ...
    """
    This is a utility for writing segmentations to nifty and npz. It requires the dataloader to have been preprocessed by
    GenericPreprocessor because it depends on the property dictionary output (dct) to know the geometry of the original
    dataloader. segmentation_softmax does not have to have the same size in pixels as the original dataloader, it will be
    resampled to match that. This is generally useful because the spacings our networks operate on are most of the time
    not the native spacings of the image dataloader.
