...
class Training(object):
    ...
    def train(self):
        if self.config.DIS:
            os.environ["CUDA_VISIBLE_DEVICES"] = ",".join(map(str, self.config.GPU_IDS))  # 例如 GPU_IDS=[0,1,2,3,4,5]
            mp.spawn(
                self._main_worker,
                args=(self.config,),
    ...
    def _main_worker(self, local_rank, config):
        ...
        if local_rank == 0:
            ...
            wandb.init(
                project=(config.WANDB.COARSE_PROJECT if self.train_type == "coarse" else config.WANDB.FINE_PROJECT),
        ...
        if self.train_type == "fine":
            train_loader, val_loader = build_loader(
                ...
                os.path.join(config.DATASET.BASE_DIR, config.DATASET.FINE.PROPRECESS_PATH),
                os.path.join(config.DATASET.BASE_DIR, config.DATASET.FINE.PROPRECESS_UL_PATH),
                config.MODEL.FINE.POOL_OP_KERNEL_SIZES,
                config.DATASET.FINE.NUM_EACH_EPOCH,
                local_rank=local_rank,
            )
            model = build_fine_model(config).to(local_rank)
            losses = {
                "ct": build_loss(config, config.MODEL.DEEP_SUPERVISION, config.MODEL.FINE.POOL_OP_KERNEL_SIZES, config.LOSS.IGNORE_EDGE),
                "mr": build_loss(config, False, config.MODEL.FINE.POOL_OP_KERNEL_SIZES, config.LOSS.IGNORE_EDGE),
            }
            if config.TRAIN.MULTI_TASK.IS_OPEN:
                losses["center_class"] = build_loss(config, False, config.MODEL.FINE.POOL_OP_KERNEL_SIZES, config.LOSS.IGNORE_EDGE, True)

        else:  # coarse
            train_loader, val_loader = build_loader(
                config,
                config.DATASET.COARSE.SIZE,
                os.path.join(config.DATASET.BASE_DIR, config.DATASET.COARSE.PROPRECESS_PATH),
                os.path.join(config.DATASET.BASE_DIR, config.DATASET.COARSE.PROPRECESS_UL_PATH),
                config.MODEL.COARSE.POOL_OP_KERNEL_SIZES,
                config.DATASET.COARSE.NUM_EACH_EPOCH,
                local_rank=local_rank,
            )
            model = build_coarse_model(config).to(local_rank)
            losses = {
                "ct": build_loss(config, config.MODEL.DEEP_SUPERVISION, config.MODEL.COARSE.POOL_OP_KERNEL_SIZES, config.LOSS.IGNORE_EDGE),
                "mr": build_loss(config, False, config.MODEL.COARSE.POOL_OP_KERNEL_SIZES, config.LOSS.IGNORE_EDGE),
            }
            ...
            with torch.no_grad():
                for idx, image_dict in enumerate(tbar):
                    ...
                    if self.train_type == "coarse-fine":
                        ...
                        raw_image_max = np.max(raw_image)
                        if raw_image_max > 0:
                            raw_image = raw_image / raw_image_max
