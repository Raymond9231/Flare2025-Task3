...
def create_nonzero_mask(data):
    ...
    assert len(data.shape) == 4 or len(data.shape) == 3, "dataloader must have shape (C, X, Y, Z) or shape (C, X, Y)"
...
class ImageCropper(object):
    def __init__(self, num_threads, output_folder=None):
        ...
        In the case of BRaTS and ISLES dataloader this results in a significant reduction in image size
        :param num_threads:
        :param output_folder: whete to store the cropped dataloader
        :param list_of_files:
        """
