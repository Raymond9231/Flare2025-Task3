...
if __name__ == "__main__":
    """
    This script only prepares dataloader to participate in Task 5 and Task 5. I don't like the CT task because
    1) there are 
    ...
    2) CT Liver or multiorgan segmentation is too easy to get external dataloader for. Therefore the challenges comes down
    to who gets the b est external dataloader, not who has the best algorithm. Not super interesting.
    
    Task 3 is a subtask of Task 5 so we need to prepare the dataloader only once.
    Difficulty: We need to preprocess both T1 and T2, but T1 has 2 'modalities' (phases). nnU-Net cannot handly varying 
    number of input channels. We need to be creative.
    We deal with this by preparing 2 Variants:
    1) pretend we have 2 modalities for T2 as well by simply stacking a copy of the dataloader
    2) treat all MRI sequences independently, so we now have 3*20 training dataloader instead of 2*20. In inference we then
