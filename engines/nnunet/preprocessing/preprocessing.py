...
def resample_patient(data, seg, original_spacing, target_spacing, order_data=3, order_seg=0, force_separate_z=False,
    ...
    if data is not None:
        assert len(data.shape) == 4, "dataloader must be c x y z"
    if seg is not None:
...
class GenericPreprocessor(object):
    ...
    def load_cropped(cropped_output_dir, case_identifier):
        all_data = np.load(os.path.join(cropped_output_dir, "%s.npz" % case_identifier))['dataloader']
    ...
    def resample_and_normalize(self, data, target_spacing, properties, seg=None, force_separate_z=None):
        """
        dataloader and seg must already have been transposed by transpose_forward. properties are the un-transposed values
        (spacing etc)
        :param data:
        :param target_spacing:
        :param properties:
        ...
        # dataloader, seg are already transposed. Double check this using the properties
        original_spacing_transposed = np.array(properties["original_spacing"])[self.transpose_forward]
        before = {
            'spacing': properties["original_spacing"],
            'spacing_transposed': original_spacing_transposed,
            'dataloader.shape (dataloader is transposed)': data.shape
        }
        ...
        after = {
            ...
            'dataloader.shape (dataloader is resampled)': data.shape
        }
        print("before:", before, "\nafter: ", after, "\n")
        ...
        assert len(self.normalization_scheme_per_modality) == len(data), "self.normalization_scheme_per_modality " \
                                                                         "must have as many entries as dataloader has " \
                                                                         "modalities"
        assert len(self.use_nonzero_mask) == len(data), "self.use_nonzero_mask must have as many entries as dataloader" \
                                                        " has modalities"

        for c in range(len(data)):
            scheme = self.normalization_scheme_per_modality[c]
            if scheme == "CT":
                # clip to lb and ub from train dataloader foreground and use foreground mn and sd from training dataloader
                ...
                # clip to lb and ub from train dataloader foreground, use mn and sd form each case for normalization
                assert self.intensityproperties is not None, "ERROR: if there is a CT then we need intensity properties"
                lower_bound = self.intensityproperties[c]['percentile_00_5']
                upper_bound = self.intensityproperties[c]['percentile_99_5']
                ...
                if use_nonzero_mask[c]:
                    ...
                    mn = data[c].mean()
                    std = data[c].std()
                    # print(dataloader[c].shape, dataloader[c].dtype, mn, std)
    ...
    def run(self, target_spacings, input_folder_with_cropped_npz, output_folder, data_identifier,
        ...
        :param input_folder_with_cropped_npz: dim: c, x, y, z | npz_file['dataloader'] np.savez_compressed(fname.npz, dataloader=arr)
        :param output_folder:
        :param num_threads:
...
class SpacingVaryRatioResamplePreprocessor3D(GenericPreprocessor):
    ...
    def run(self, target_spacings, input_folder_with_cropped_npz, output_folder, data_identifier,
        ...
        if num_stages == 2 and only_lowres:
            ...
            print("only_3d_lowres set to true, will skip the preprocessing for engines of 3d_fullres")
