...
from engines.dataloader.utils_extended import (change_axes_of_image,
...
target_labels = np.array([1, 2, 3, 13])
label_same = np.array([0, 1, 2, 3])
def get_z_bounds_vectorized(seg_data, labels):
    """Vectorized z-bounds calculation"""
    # Create boolean mask for all target labels at once
    ...
    #print(z_min, z_max)
    lower = max(z_min - 2, 0)
    upper = min(z_max + 4, seg_data.shape[0])
    return lower, upper
...
class model_train_data_process(object):
    ...
    def process(self, image_id):
        ...
        data_id1 = data_id.replace("CT2PET_","")
        ...
        mask, _, mask_direction, label_itk_info = load_data(
            join(self.mask_path, data_id1 + ".nii.gz")
        )
        #print(image_direction.flatten().tolist())
        ...
        if self.is_abdomen_crop:
            lower, upper = get_z_bounds_vectorized(mask, target_labels)
            image = image[lower:upper, :, :]
            mask = mask[lower:upper, :, :]
            # Vectorized label processing
            seg_mask = np.isin(mask, target_labels)
            mask = np.where(seg_mask, mask, 0)
            seg_same_mask = np.isin(mask, label_same)
            mask = np.where(seg_same_mask, mask, 4)
        #
        # image = image.transpose(1, 2, 0)    # zyx->yxz
        ...
        if self.nor_dir:
            #print(image_itk_info['direction'])
            image = change_axes_of_image(image, image_direction)
            #image, label_itk_info['direction'] = change_axes_of_image(image, image_direction, label_itk_info['direction'])
            mask = change_axes_of_image(mask, mask_direction)
            if is_softmax_exists:
                for s_i in range(softmax_image.shape[0]):
                    softmax_image[s_i], _ = change_axes_of_image(
        ...
        if "fine" in self.train_type:
            ...
            image_nii = sitk.GetImageFromArray(crop_image_normal.astype(np.float32))
            image_nii.SetSpacing(data_info_crop["resize_crop_spacing"][::-1])
            sitk.WriteImage(image_nii, os.path.join(self.preprocess_fine_path, f"{data_id}.nii.gz"))


            mask_nii = sitk.GetImageFromArray(crop_mask_resize.astype(np.int8))
            mask_nii.SetSpacing(data_info_crop["resize_crop_spacing"][::-1])
            sitk.WriteImage(mask_nii, os.path.join(self.preprocess_fine_path, f"{data_id}_mask.nii.gz"))
            np.savez_compressed(
...
if __name__ == "__main__":
    cfg = "/home/raymond9231/flare/FLARE2025/configs/preprocess/preprocess_step1_RealPET.yaml"
    _, config = parse_option("other", cfg)
