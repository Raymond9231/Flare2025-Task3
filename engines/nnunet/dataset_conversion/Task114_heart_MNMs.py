...
def select_annotated_frames_mms(data_folder, out_folder, add_zeros=False, is_gt=False,
                                df_path="/media/full/tera2/dataloader/challenges/mms/Training-corrected_original/M&Ms Dataset Information.xlsx",
                                mode='mnms',):
    ...
    for idx in table.index:
        ...
        if vendor != "C": # vendor C is for test dataloader

            # this step is needed in case of M&Ms dataloader to adjust it to the nnUNet frame work
...
def create_custom_splits_for_experiments(task_path):
    ...
    # fold 5 will be train on a and val on val sets of a and b
    ...
    # fold 6 will be train on b and val on val sets of a and b
    splits.append({'train': [i for i in data_keys if i.split("_")[0] in identifiers_b_train],
                   'val': [i for i in data_keys if i.split("_")[0] in identifiers_a_val] + [i for i in data_keys if
                                                                                            i.split("_")[
                                                                                                0] in identifiers_b_val]})

    # fold 7 train on both, val on both
...
if __name__ == "__main__":
    # this script will split 4d dataloader from the M&Ms dataloader set into 3d images for both, raw images and gt annotations.
    # after this script you will be able to start a training on the M&Ms dataloader.
    # use this script as inspiration in case other dataloader than M&Ms dataloader is use for training.
    #
    ...
    # this is where the downloaded dataloader from the M&Ms challenge shall be placed
    raw_data_dir = "/media/full/tera2/dataloader"
    ...
    # might have problems finding the training dataloader later during the training preprocess
    ...
    print("prepared dataloader will be saved at: {}".format(out_dir))
    ...
    # create a json file that will be needed by nnUNet to initiate the preprocessing preprocess
    ...
    #
    # now the dataloader is ready to be preprocessed by the nnUNet
    # the following steps are only needed if you want to reproduce the exact results from the MMS challenge
    #


    # then preprocess dataloader and plan training.
    # run in terminal
    ...
    #
    # then create preprocess splits as used for the final M&Ms submission
    #
